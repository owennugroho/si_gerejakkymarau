...
const props = defineProps({ jadwalTurnes: Array })
function destroy(id) {
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex justify-between items-center">
        ...
        <Link :href="route('admin.jadwal-turne.create')" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
          + Tambah Jadwal
        </Link>
      </div>
    </template>
      ...
      <div class="overflow-x-auto bg-white shadow rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Stasi / Lokasi</th>
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="item in props.jadwalTurnes" :key="item.id">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ item.stasi.nama_stasi }}</td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ item.jam_mulai }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ item.romo.nama }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                <Link :href="route('admin.jadwal-turne.show', item.id)" class="text-indigo-600 hover:text-indigo-900">Lihat</Link>
                <Link :href="route('admin.jadwal-turne.edit', item.id)" class="text-yellow-600 hover:text-yellow-900">Edit</Link>
